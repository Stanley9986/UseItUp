...
- Migrations run through 018 (recipe source-language column, content-addressed translation cache, OpenAI food-image provider allowance, Edge Function rate-limit storage, pantry item source-language column).
...
- Make the local food dictionary multidirectional. It is currently English-keyed and only translates English-source names; a non-English-source name (e.g. a Chinese-entered item viewed in Japanese, or any non-English name viewed in English) bypasses the dictionary and falls to the LLM. Build a reverse index (each language's translations mapping back to the English key, including the synonym/alias entries) so any source language can be normalized to the English key and then translated to the target locally, with English used as the pivot. Handle script-specific normalization (e.g. Chinese has no spaces/plurals) and multiple valid names per language (番茄 vs 西红柿 for tomato). This builds on the pantry item source-language tag now in place.
