...
- Most mock data wiring has been removed; profile preferences are still placeholder-level.

## Section 2 - Tech Debt / Cleanup

- Fix info-vs-error message styling so empty-state nudges do not reuse red error styling.
- Squash migration 005 into the later favorite recipe migration before shipping or presenting a fresh setup path.
- Centralize duplicated `getErrorMessage` helpers into a shared error utility.
- Finish mock-data removal, especially profile preference placeholders.
...
- Save dietary preferences and feed them into recipe generation.
